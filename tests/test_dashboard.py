...
def test_new_user(setup):
    ...
    dash_board.username_key("test_user@15")
    dash_board.enter_password("test_user@15")
    dash_board.confirm_password("test_user@15")
    dash_board.click_save()
    assert dash_board.verify_success_message().is_displayed()
    login_page.click_logout()
    login_page.login("test_user@15", "test_user@15")
...
def test_leave_apply(setup):
    ...
    dash_board.from_leave_dates("2026-10-06")
    dash_board.to_leave_dates("2026-11-06")
...
def test_emp_claim(setup):
    login_page = LoginPage(setup)
    dashboard_page = DashBoardpage(setup)
    login_page.login("test_user@16", "test_user@15")
